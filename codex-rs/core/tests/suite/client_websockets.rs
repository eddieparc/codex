...
use codex_core::CodexRequestIdentity;
...
async fn responses_websocket_request_prewarm_reuses_connection() {
    ...
    let mut client_session = harness.client.new_session();
    let prompt = prompt_with_input(vec![message_item("hello")]);
    let request_identity = harness.client.request_identity(/*turn_id*/ None);
    client_session
        .prewarm_websocket(
            &prompt,
            &harness.model_info,
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let request_identity = harness.client.request_identity(Some("turn-123"));

    stream_until_complete_with_request_identity(
        &mut client_session,
        &harness,
        &prompt_one,
        /*service_tier*/ None,
...
async fn stream_until_complete_with_request_identity(
    client_session: &mut ModelClientSession,
    harness: &WebsocketTestHarness,
    prompt: &Prompt,
    service_tier: Option<ServiceTier>,
